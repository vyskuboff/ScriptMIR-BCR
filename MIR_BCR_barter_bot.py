...
def process_sms(sms_data, mysql_conn, pg_conn):
    ...
    if mir_card_match:
        identifier = mir_card_match.group(1)
        phone_number = find_phone_by_mir_card(identifier, pg_conn)
        is_mir_card = True
    elif mir_account_match:
        identifier = mir_account_match.group(1)
        phone_number = find_phone_by_account(identifier, pg_conn)
...
def connect_to_mysql(mysql_params, max_retries=10, delay=2):
    retries = 0
